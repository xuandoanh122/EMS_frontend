...
export const classroomsApi = {
  ...
  getEnrollments: async (class_code: string, params: { page?: number; page_size?: number } = {}): Promise<APIResponse<EnrollmentListResponse>> => {
    const { data } = await apiClient.get(`/api/v1/classrooms/${class_code}/enrollments`, { params })
    return data
  },

  addEnrollment: async (class_code: string, payload: EnrollmentCreateRequest): Promise<APIResponse<unknown>> => {
    const { data } = await apiClient.post(`/api/v1/classrooms/${class_code}/enrollments`, payload)
    return data
  },

  removeEnrollment: async (class_code: string, student_code: string): Promise<APIResponse<null>> => {
    const { data } = await apiClient.delete(`/api/v1/classrooms/${class_code}/enrollments/${student_code}`)
    return data
  },
}
